                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend; i++)
    {
        if(vMatches12[i]>=0)
        {
            //pair [ref frame keypoint id, current frame keypoint id]
            mvMatches12.push_back(make_pair(i,vMatches12[i]));
            mvbMatched1[i]=true;
        }
        else /// -1 means not matched
            mvbMatched1[i]=false;
    }
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        {
            /// get a random int number between [0, avalible_index]
